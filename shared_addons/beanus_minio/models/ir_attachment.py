...
class MinioAttachment(models.Model):
    ...
    def _get_minio_client(self):
        ...
        _logger.debug(f"endpoint={host} - region={region} - access_key: {access_key} - secret_key: {secret_key}")
        return Minio(endpoint=f"{host}",
    ...
    @api.model
    def store_file_read(self, fname, bin_size=False):
        return self._store_file_read(self, fname, bin_size)
